...
if ($_SERVER['REQUEST_METHOD'] !== 'POST')
    dl_error("方法错误", "请不要直接访问此页面或使用 GET 方式访问！"); // 方法错误
if (
    (isset($_POST["fs_id"]) &&
        isset($_POST["time"]) &&
        isset($_POST["sign"]) &&
...
if ($json4["errno"] !== 0) {
    ...
    if (isset($error[$json4["errno"]]))
        dl_error($error[$json4["errno"]][0], $error[$json4["errno"]][1]);
    else
        dl_error("获取下载链接失败", "未知错误！<br />错误号：" . $json4["errno"], true); // 未知错误
}
        ...
        <div class="col-md-7 col-sm-8 col-11">
            <div class="alert alert-primary" role="alert">
                <h5 class="alert-heading">
                    <?php echo Language["DownloadLinkSuccess"]; ?>
                </h5>
                <hr />
                <?php
                if (USING_DB) {
                    if ($usingcache)
                        echo "<p class=\"card-text\">下载链接从数据库中提取，不消耗免费解析次数。</p>";
                    else
                        echo "<p class=\"card-text\">服务器将保存下载地址" . DownloadLinkAvailableTime . "小时，时限内再次解析不消耗免费次数。</p>";
                }
                ...
                if (strstr('https://' . $realLink, "//qdall"))
                    echo '<h5 class="text-danger">当前SVIP账号已被限速，请联系站长更换账号。</h5>';
                ...
                ?>
                <p class="card-text">
                    <a href="javascript:void(0)" data-toggle="modal" data-target="#SendToAria2">
                        <?php echo Language["SendToAria2"]; ?>(Motrix)
                    </a>
                    <a href="" id="filecxx" style="display: none;">
                        <?php echo Language["SendToFilecxx"]; ?>
                    </a>
                </p>
                <script>
                    try {
                ...
                <p class="card-text"><a href="?help" target="_blank">
                        <?php echo Language["DownloadLink"] . Language["HelpButton"]; ?>（必读）
                    </a></p>
                <p class="card-text">
                    <?php echo Language["DownloadTip"]; ?>
                </p>
                        ...
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title">
                                    <?php echo Language["SendToAria2"]; ?> Json-RPC
                                </h5>
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn btn-primary" onclick="addUri()" data-dismiss="modal">
